...
//This version caches and has other additional properties that the SimpleMCTSGamer does not
//It is about 10% faster than the simple gamer due to caching and has more flexibility in opponent modeling.
public class GenericMCTSGamer extends SampleGamer {
	 ...
	 *Values around 20 are good if we use the terminal state goal (win ratio=false).
	 *Values around 1 are good if we use the win ratio.*/
	...
	private void initializeVariables() throws MoveDefinitionException {
	...
	public void runMCTS() throws MoveDefinitionException, TransitionDefinitionException, GoalDefinitionException {
		while( System.currentTimeMillis() < finishBy ) {
			...
			double[] scores = simulation(newGrandchild);
	...
	public double selectfn(Node node, boolean opponent) {
		if(useMiniMax) { //Here the opponent tries to hurt us
			...
			if(opponent) //Here the opponent tries to better himself
				return node.oppAvgUtility + explorationConstant*Math.sqrt(2*Math.log(node.parent.visits)/node.visits);
			else
				return node.avgUtility + explorationConstant*Math.sqrt(2*Math.log(node.parent.visits)/node.visits);
		}
	}
	...
	public double[] simulation(Node root) throws GoalDefinitionException, TransitionDefinitionException, MoveDefinitionException {
		if(root.isMax)
			logger.log(Level.WARNING, "Error. Should only simulate on the Max Nodes");

		double[] result = new double[2]; //First entry is self score, 2nd entry is avg opponent score
